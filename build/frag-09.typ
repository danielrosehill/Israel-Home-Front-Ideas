...
- Closely related to #link("../info-during-after/current-guidance-field-in-feed.md")[Add a \"current guidance\" field to the alert data feed];. The current-guidance field is the kind of schema improvement that a properly versioned, documented public API would make easy to ship — and conversely, a public API without an unambiguous state field would just propagate today\'s ambiguity to more consumers.
