...
#quote(block: true)[
#strong[Layer 1 of the public shelter stack] — the institution, the physical standard, and the canonical national register. The data schema layer is in #link("standard-listing-format-and-wayfinding.md")[Standard listing format and physical wayfinding];; the consumer-app layer is in #link("municipal-shelter-finder-app.md")[Mandatory municipal shelter-finder app];.
]
...
- #strong[National GIS register.] An orderly, authoritative, geo-referenced register of every public shelter in the country, with status, capacity, accessibility, and last-inspection data. Publicly queryable \(respecting any legitimate security constraints).
- #strong[A minimum code — beyond structural integrity.] Not just \"will it survive a blast\" but \"is it viable for human beings to spend hours or days in.\" Running water, ventilation, summer cooling, lighting, sanitation, seating, basic first aid.
- #strong[Accessibility & medical baseline.] A mandatory minimum feature set that accommodates not just the able-bodied but also the elderly, people with disabilities, and those with medical conditions \(e.g. step-free access, a place to sit or lie down, power outlets for medical devices).
- #strong[Redundant connectivity.] Every public shelter should have at least one — preferably multiple independent — sources of connectivity for failover. An ethernet port in the wall does not count: it relies on someone arriving with a laptop or a router, which often does not happen.
- #strong[Hardened alert tablet in every shelter.] Each shelter should have a wall-mounted, MDM-managed tablet locked down to display only Pikud HaOref and WEA \(Wireless Emergency Alert) messages. Powered from mains with battery backup, on its own SIM or equivalent, so that people inside can see alerts and all-clears even if nobody present has a working phone.
- #strong[Inspection and enforcement.] Routine inspections against the code, with published results and a clear remediation path when a shelter falls below standard.

== Related ideas \(promoted to their own files)
<related-ideas-promoted-to-their-own-files>
- #link("comms-redundancy-in-shelters.md")[Communications redundancy as a baseline duty] — cellular, Wi-Fi, hardened WEA tablet, and pre-tuned AM/FM radio in every shelter.
- #link("long-stay-amenities.md")[Long-stay amenities — fewer but better-equipped shelters] — mattresses, eye masks, earplugs, AC, and the trade-off this implies.

== Related sub-ideas
<related-sub-ideas>
=== Prohibit shelter operators from barring or obstructing access
<prohibit-shelter-operators-from-barring-or-obstructing-access>
#emph[Captured: 2026-04-06]
...
== Notes
<notes>
- The GIS register alone would unlock a lot of downstream value — apps could route people to the nearest #emph[viable] shelter, not just the nearest one on paper.
